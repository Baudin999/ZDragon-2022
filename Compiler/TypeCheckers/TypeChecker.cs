...
public class TypeChecker
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(nodeReference.From))
        {
            var from = _zdragon.Get(nodeReference.From);
            if (from is null) _zdragon.Errors.Add(new Error(nodeReference.FromToken, @$"Type '{nodeReference.From}' does not exist"));
        }
